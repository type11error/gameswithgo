...
func (tex *texture) drawBinlinearScaled(scaleX, scaleY float32, pixels []byte) {
  newWidth := int(float32(tex.w)*scaleX)
  newHeight := int(float32(tex.h)*scaleY)
  texW4 := tex.w*4
  for y := 0; y < newHeight; y++ {
...
func main() {
  ...
  cloudTexture := texture{pos{0,0}, cloudPixels, winWidth, winHeight, winWidth * 4, 0}
  ...
  for running {
    ...
    for _, t := range balloonTextures {
      t.drawBinlinearScaled(t.scale, t.scale, pixels)
    }
